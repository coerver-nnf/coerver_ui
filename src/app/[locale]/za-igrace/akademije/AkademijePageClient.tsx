...
const trainingPhaseTimes = ["15 %", "10 %", "15 %", "25 %", "30 %", "5 %"];

export default function AkademijePageClient({ academies }: { academies: Academy[] }) {
  const t = useTranslations("players.academies");
  const [activeLocation, setActiveLocation] = useState(0);

  const trainingPhases = trainingPhaseTimes.map((time, index) => ({
    time,
    title: t(`structure.phases.${index}.title`),
    description: t(`structure.phases.${index}.description`),
  }));
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-flex items-center gap-2 bg-coerver-green/20 rounded-full px-4 py-2 mb-6">
                <Link href="/za-igrace" className="text-white/60 hover:text-white text-sm">{t("breadcrumbParent")}</Link>
                <span className="text-white/40">/</span>
                <span className="text-coerver-green text-sm font-semibold">{t("breadcrumbCurrent")}</span>
              </div>

              <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-white leading-tight mb-6">
                Coerver<span className="text-coerver-green">®</span> {t("heroTitle")}
              </h1>
            ...
            <div className="hidden lg:grid grid-cols-2 gap-4">
              {[
                { value: academies.length.toString(), label: t("stats.locations") },
                { value: uniqueCities.toString(), label: t("stats.cities") },
                { value: "12", label: t("stats.maxPlayers") },
                { value: "75", label: t("stats.minutes") },
              ].map((stat, index) => (
        ...
        <div className="container mx-auto px-6 lg:px-8">
          <div className="animate-on-scroll text-center max-w-3xl mx-auto mb-16">
            <div className="inline-flex items-center gap-2 bg-coerver-green/10 rounded-full px-4 py-2 mb-6">
              <span className="text-coerver-green text-sm font-semibold">{t("locations.badge")}</span>
            </div>
          ...
          {academies.length === 0 ? (
            <div className="text-center py-20">
              <p className="text-gray-500 text-lg">{t("locations.empty")}</p>
            </div>
          ) : (
            <>
                          ...
                          {academy.schedule && (
                            <div className="bg-white rounded-xl p-4">
                              <div className="flex items-center gap-3 mb-2">
                                ...
                                <span className="font-semibold text-coerver-dark text-sm">{t("locations.schedule")}</span>
                              </div>
                          ...
                          {academy.age_groups && academy.age_groups.length > 0 && (
                            <div className="bg-white rounded-xl p-4">
                              <div className="flex items-center gap-3 mb-2">
                                ...
                                <span className="font-semibold text-coerver-dark text-sm">{t("locations.ageGroups")}</span>
                              </div>
                      ...
                      {academies[activeLocation].schedule && (
                        <div className="bg-white rounded-2xl p-5">
                          <div className="flex items-center gap-3 mb-3">
                            ...
                            <span className="font-semibold text-coerver-dark">{t("locations.schedule")}</span>
                          </div>
                      ...
                      {academies[activeLocation].age_groups && academies[activeLocation].age_groups!.length > 0 && (
                        <div className="bg-white rounded-2xl p-5">
                          <div className="flex items-center gap-3 mb-3">
                            ...
                            <span className="font-semibold text-coerver-dark">{t("locations.ageGroups")}</span>
                          </div>
                          <div className="flex flex-wrap gap-2">
      ...
      <section className="py-24 lg:py-32 bg-coerver-dark relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 lg:px-8 relative">
          <div className="animate-on-scroll text-center max-w-3xl mx-auto mb-16">
            ...
            <div className="inline-flex items-center gap-2 bg-coerver-green/20 rounded-full px-4 py-2 mb-6">
              <span className="text-coerver-green text-sm font-semibold">{t("structure.badge")}</span>
            </div>
        ...
        <div className="container mx-auto px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-start">
            <div className="animate-on-scroll">
              ...
              <div className="inline-flex items-center gap-2 bg-coerver-green/10 rounded-full px-4 py-2 mb-6">
                <span className="text-coerver-green text-sm font-semibold">{t("inquiry.badge")}</span>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl">
                  ...
                  <div>
                    <div className="font-semibold text-coerver-dark">{t("inquiry.email")}</div>
                ...
                <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl">
                  ...
                  <div>
                    <div className="font-semibold text-coerver-dark">{t("inquiry.phone")}</div>
            ...
            <div className="animate-on-scroll">
              <InquiryForm type="academy" title={t("inquiry.formTitle")} />
            </div>
          </div>
        </div>
      </section>
