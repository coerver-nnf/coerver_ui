...
function CountdownTimer({ deadline }: { deadline: string }) {
  const t = useTranslations("players.camps");
...
function QuickInquiryForm() {
  ...
  if (isSubmitted) {
    return (
      <div className="bg-coerver-green/10 rounded-2xl p-6 text-center">
        ...
        <h4 className="font-bold text-coerver-dark mb-1">{t("quickForm.thanks")}</h4>
        <p className="text-sm text-gray-600">{t("quickForm.reply")}</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      >
        {isSubmitting ? t("quickForm.sending") : t("quickForm.send")}
      </button>
    </form>
  );
}
...
const testimonialNames = ["Marina K.", "Tomislav B.", "Ana M."];
...
function CampCard({ camp, featured = false }: { camp: Camp; featured?: boolean }) {
  ...
  const availablePrices = [camp.price_day_only, camp.price_full_day, camp.price].filter((p): p is number => p !== null && p > 0);
      ...
      <div className="absolute top-4 right-4 z-10 flex flex-col gap-2 items-end">
        {(isAlmostFull || isFull) && (
          <span className={cn(
            ...
            {isFull ? t("card.full") : t("card.spotsLeftUrgent", { count: spotsLeft })}
          </span>
        )}
        {!isFull && viewingCount > 0 && (
          <span className="inline-flex items-center gap-1.5 bg-white/90 backdrop-blur-sm text-gray-700 text-xs font-medium px-3 py-1.5 rounded-full shadow-sm">
            ...
            {t("card.viewing", { count: viewingCount })}
          </span>
        )}
      </div>
          ...
          {displayPrice && (
            <div className="absolute bottom-4 left-4">
              <div className="bg-white rounded-2xl px-4 py-2 shadow-lg">
                <div className="text-xs text-gray-500">{t("card.from")}</div>
          ...
          {daysUntil > 0 && daysUntil <= 30 && (
            <div className="absolute bottom-4 right-4">
              <div className="bg-coerver-dark/90 backdrop-blur-sm text-white rounded-xl px-3 py-2 text-center">
                ...
                <div className="text-xs opacity-80">{t("card.daysUntil")}</div>
              </div>
            </div>
          )}
        </div>
        ...
        <div className={cn("p-6 lg:p-8", featured ? "" : "lg:col-span-2")}>
          ...
          <div className="grid grid-cols-2 gap-3 mb-5">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-xs text-gray-500">{t("card.date")}</div>
            ...
            {camp.location && (
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-xs text-gray-500">{t("card.location")}</div>
            ...
            {camp.age_groups && camp.age_groups.length > 0 && (
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-xs text-gray-500">{t("card.age")}</div>
                  <div className="text-sm font-semibold text-coerver-dark">{t("card.ageValue", { ages: camp.age_groups.join(", ") })}</div>
                </div>
              </div>
            )}
            {spotsLeft > 0 && (
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-xs text-gray-500">{t("card.spots")}</div>
                  <div className="text-sm font-semibold text-coerver-dark">{t("card.spotsFree", { count: spotsLeft })}</div>
                </div>
              </div>
            )}
          </div>
          ...
          {totalSpots > 0 && spotsPercentage > 0 && (
            <div className="mb-5">
              ...
              <div className="flex justify-between mt-1.5">
                <span className="text-xs text-gray-500">{t("card.percentFull", { percent: Math.round(spotsPercentage) })}</span>
                {spotsPercentage >= 50 && <span className="text-xs text-red-500 font-medium">{t("card.hurry")}</span>}
              </div>
            </div>
          )}
          ...
          {featured && camp.registration_deadline && (
            <div className="mb-5 bg-gradient-to-r from-red-500 to-orange-500 rounded-2xl p-4">
              <div className="flex items-center justify-between flex-wrap gap-4">
                <div>
                  <div className="text-white/80 text-sm">{t("card.deadline")}</div>
                </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            >
              {isFull ? t("card.full") : t("card.signUp")}
              {!isFull && (
        ...
        >
          {galleryImages.map((src, index) => (
            <div key={index} className="min-w-full h-full relative">
              <Image src={src} alt={t("galleryAlt", { num: index + 1 })} fill className="object-cover" />
            </div>
          ))}
        </div>
      </div>
...
export default function KampoviPageClient({ camps }: { camps: Camp[] }) {
  const t = useTranslations("players.camps");
  ...
  const testimonials = testimonialNames.map((name, index) => ({
    name,
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-flex items-center gap-2 bg-red-500/20 backdrop-blur-sm rounded-full px-4 py-2 mb-6 border border-red-500/30">
                ...
                <span className="text-red-300 text-sm font-semibold">{t("hero.badge")}</span>
              </div>

              <h1 className="text-5xl md:text-6xl lg:text-7xl font-black text-white leading-[0.95] mb-6">
                {t("hero.title1")}
                <br />
                <span className="text-coerver-green">{t("hero.title2")}</span>
              </h1>
              ...
              <div className="flex flex-wrap gap-8 mb-10">
                {trustStats.map((stat, index) => (
                  <div key={index} ref={animatedStats[index].ref}>
                    ...
                    <div className="text-white/50 text-sm">{t(`trustStats.${index}`)}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="lg:hidden mt-8 bg-white/10 backdrop-blur-sm rounded-2xl p-5 border border-white/10">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <div className="text-white font-semibold">{t("hero.questions")}</div>
                    <div className="text-white/60 text-sm">{t("hero.reply24h")}</div>
                  </div>
                  <a
              ...
              <div className="mt-12 pt-8 border-t border-white/10">
                <p className="text-white/40 text-xs uppercase tracking-wider mb-4">{t("hero.partners")}</p>
            ...
            <div className="hidden lg:block">
              <div className="bg-white rounded-3xl p-8 shadow-2xl">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h3 className="font-bold text-coerver-dark">{t("hero.quickInquiry")}</h3>
                    <p className="text-sm text-gray-500">{t("hero.reply24h")}</p>
                  </div>
                </div>
                <QuickInquiryForm />
              </div>
        ...
        <div className="container mx-auto px-6 lg:px-8">
          <div className="animate-on-scroll text-center max-w-3xl mx-auto mb-12">
            <div className="inline-flex items-center gap-2 bg-coerver-green/10 rounded-full px-4 py-2 mb-6">
              <span className="text-coerver-green text-sm font-semibold">{t("list.badge")}</span>
            </div>
          ...
          {camps.length === 0 ? (
            <div className="text-center py-16 max-w-xl mx-auto">
              ...
              <h3 className="text-2xl font-bold text-coerver-dark mb-3">{t("list.emptyTitle")}</h3>
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm max-w-sm mx-auto mb-6">
                <h4 className="font-semibold text-coerver-dark mb-4">{t("list.notifyMe")}</h4>
                <QuickInquiryForm />
              </div>
        ...
        <div className="container mx-auto px-6 lg:px-8 relative">
          <div className="animate-on-scroll text-center max-w-3xl mx-auto mb-12">
            <div className="inline-flex items-center gap-2 bg-coerver-green/20 rounded-full px-4 py-2 mb-6">
              <span className="text-coerver-green text-sm font-semibold">{t("parents.badge")}</span>
            </div>
          ...
          <div className="animate-on-scroll mt-12 text-center">
            <div className="inline-flex items-center gap-4 bg-white/10 backdrop-blur-sm rounded-full px-6 py-3">
              ...
              <span className="text-white/60 text-sm">{t("parents.googleReviews")}</span>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="grid lg:grid-cols-2 gap-16">
            ...
            <div className="animate-on-scroll">
              <div className="bg-coerver-dark rounded-3xl p-8 sticky top-8">
                <h3 className="text-2xl font-black text-white mb-4">{t("faq.cardTitle")}</h3>
                ...
                <div className="space-y-4 mb-8">
                  ...
                  >
                    ...
                    <div>
                      <div className="font-semibold">{t("faq.email")}</div>
                  ...
                  >
                    ...
                    <div>
                      <div className="font-semibold">{t("faq.phone")}</div>
        ...
        <div className="container mx-auto px-6 lg:px-8 relative text-center">
          <div className="max-w-3xl mx-auto">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              ...
              >
                <span>{t("finalCta.signUpNow")}</span>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" />
                </svg>
              </a>
