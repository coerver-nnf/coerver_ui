...
export default function EditAcademyPage({
  ...
  const { translations, setTranslations, getField, setField, completeness } =
    useTranslationsDraft();
  ...
  async function loadAcademy() {
    ...
    try {
      ...
      setTranslations(academy.translations || {});
      reset({
        name: academy.name,
        slug: academy.slug,
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        {trLocale && (
          <div className="bg-white rounded-xl border border-coerver-gray-200 p-6 space-y-4">
            ...
            <Input
              label="Naziv"
              value={getField(trLocale, "name")}
              onChange={(e) => setField(trLocale, "name", e.target.value)}
            ...
            <div>
              <Textarea
                label="Opis"
                rows={4}
                value={getField(trLocale, "description")}
                onChange={(e) =>
                  setField(trLocale, "description", e.target.value)
                }
                placeholder={watch("description") || ""}
              />
              <CopyFromHrButton
                onClick={() =>
                  setField(trLocale, "description", watch("description") || "")
                }
              />
            </div>
            ...
            <Input
              label="Grad/Mjesto"
              value={getField(trLocale, "location")}
              onChange={(e) => setField(trLocale, "location", e.target.value)}
            ...
            <div>
              <Textarea
                label="Raspored"
                rows={3}
                value={getField(trLocale, "schedule")}
                onChange={(e) => setField(trLocale, "schedule", e.target.value)}
                placeholder={watch("schedule") || ""}
              />
              <CopyFromHrButton
                onClick={() =>
                  setField(trLocale, "schedule", watch("schedule") || "")
                }
              />
            </div>
          </div>
