...
export default function BlogCategoriesPage() {
  ...
  const { translations, setTranslations, getField, setField } =
    useTranslationsDraft();
  ...
  async function onSubmit(data: CategoryFormData) {
    ...
    try {
      const payload = { ...data, translations: cleanTranslations(translations) };
      if (editingCategory) {
    ...
    <div className="space-y-6">
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="border-t border-coerver-gray-200 pt-4 space-y-4">
            <p className="text-sm text-coerver-gray-500">
              Prijevodi naziva (prazno polje koristi hrvatski naziv)
            </p>

            <Input
              label="Naziv (SL)"
              value={getField("sl", "name")}
              onChange={(e) => setField("sl", "name", e.target.value)}
              placeholder={name || ""}
            />

            <Input
              label="Naziv (EN)"
              value={getField("en", "name")}
              onChange={(e) => setField("en", "name", e.target.value)}
              placeholder={name || ""}
            />
          </div>
