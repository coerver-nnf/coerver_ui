"use client";

import { useState, useEffect } from "react";
import { useRouter } from "next/navigation";
import Link from "next/link";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { z } from "zod";
import { Button } from "@/components/ui/Button";
import { Input, Textarea, Select } from "@/components/ui/Input";
import { RichTextEditor, ImageUpload } from "@/components/admin/forms";
import { LocaleTabs, type EditLocale } from "@/components/admin/forms/LocaleTabs";
import {
  useTranslationsDraft,
  cleanTranslations,
  CopyFromHrButton,
} from "@/components/admin/forms/useTranslationsDraft";
import { createPost, getBlogCategories, BlogCategory } from "@/lib/api/posts";
import { slugify } from "@/lib/utils";

const postSchema = z.object({
  title: z.string().min(1, "Naslov je obavezan"),
  slug: z.string().min(1, "Slug je obavezan"),
  excerpt: z.string().optional(),
  content: z.string().min(1, "Sadržaj je obavezan"),
  featured_image: z.string().optional(),
  category_id: z.string().optional(),
  status: z.enum(["draft", "published"]),
});

type PostFormData = z.infer<typeof postSchema>;

export default function NewPostPage() {
  const router = useRouter();
  const [categories, setCategories] = useState<BlogCategory[]>([]);
  const [content, setContent] = useState("");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [editLocale, setEditLocale] = useState<EditLocale>("hr");
  const [trContentKey, setTrContentKey] = useState(0);
  const { translations, getField, setField, completeness } =
    useTranslationsDraft();
  const trLocale = editLocale === "hr" ? null : editLocale;

  const {
    register,
    handleSubmit,
    watch,
    setValue,
    formState: { errors },
  } = useForm<PostFormData>({
    resolver: zodResolver(postSchema),
    defaultValues: {
      status: "draft",
      content: "",
    },
  });

  const title = watch("title");
  const featuredImage = watch("featured_image");

  useEffect(() => {
    loadCategories();
  }, []);

  useEffect(() => {
    if (title) {
      setValue("slug", slugify(title));
    }
  }, [title, setValue]);

  useEffect(() => {
    setValue("content", content);
  }, [content, setValue]);

  async function loadCategories() {
    try {
      const data = await getBlogCategories();
      setCategories(data);
    } catch (error) {
      console.error("Error loading categories:", error);
    }
  }

  async function onSubmit(data: PostFormData) {
    setIsSubmitting(true);
    try {
      await createPost({
        ...data,
        translations: cleanTranslations(translations),
      });
      router.push("/dashboard/admin/blog");
    } catch (error) {
      console.error("Error creating post:", error);
    } finally {
      setIsSubmitting(false);
    }
  }

  const categoryOptions = [
    { value: "", label: "Odaberi kategoriju" },
    ...categories.map((cat) => ({ value: cat.id, label: cat.name })),
  ];

  const statusOptions = [
    { value: "draft", label: "Skica" },
    { value: "published", label: "Objavi" },
  ];

  return (
    <div className="max-w-4xl space-y-6">
      {/* Header */}
      <div className="flex items-center gap-4">
        <Link
          href="/dashboard/admin/blog"
          className="p-2 hover:bg-coerver-gray-100 rounded-lg transition-colors"
        >
          <svg className="w-5 h-5 text-coerver-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
          </svg>
        </Link>
        <h1 className="text-2xl font-bold text-coerver-gray-900">Novi članak</h1>
      </div>

      <LocaleTabs
        value={editLocale}
        onChange={setEditLocale}
        completeness={completeness}
      />

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className={editLocale === "hr" ? "space-y-6" : "hidden"}>
        {/* Basic Info */}
        <div className="bg-white rounded-xl border border-coerver-gray-200 p-6 space-y-4">
          <Input
            label="Naslov"
            {...register("title")}
            error={errors.title?.message}
            placeholder="Unesite naslov članka"
          />

          <Input
            label="Slug"
            {...register("slug")}
            error={errors.slug?.message}
            helperText="URL putanja (automatski generirana)"
          />

          <Textarea
            label="Sažetak"
            {...register("excerpt")}
            error={errors.excerpt?.message}
            placeholder="Kratki sažetak članka..."
            rows={2}
          />

          <div className="grid grid-cols-2 gap-4">
            <Select
              label="Kategorija"
              {...register("category_id")}
              options={categoryOptions}
            />

            <Select
              label="Status"
              {...register("status")}
              options={statusOptions}
            />
          </div>
        </div>

        {/* Featured Image */}
        <div className="bg-white rounded-xl border border-coerver-gray-200 p-6">
          <ImageUpload
            label="Naslovna slika"
            value={featuredImage}
            onChange={(url) => setValue("featured_image", url || "")}
            folder="blog"
            aspectRatio="wide"
          />
        </div>

        {/* Content */}
        <div className="bg-white rounded-xl border border-coerver-gray-200 p-6">
          <RichTextEditor
            label="Sadržaj"
            content={content}
            onChange={setContent}
            error={errors.content?.message}
          />
        </div>
        </div>

        {/* Translations (SL/EN) */}
        {trLocale && (
          <div className="bg-white rounded-xl border border-coerver-gray-200 p-6 space-y-4">
            <div>
              <h2 className="text-lg font-semibold text-coerver-gray-900">
                Prijevod ({trLocale.toUpperCase()})
              </h2>
              <p className="text-sm text-coerver-gray-500 mt-1">
                Prazna polja na javnoj stranici koriste hrvatski sadržaj.
              </p>
            </div>

            <Input
              label="Naslov"
              value={getField(trLocale, "title")}
              onChange={(e) => setField(trLocale, "title", e.target.value)}
              placeholder={watch("title") || ""}
            />

            <div>
              <Textarea
                label="Sažetak"
                rows={2}
                value={getField(trLocale, "excerpt")}
                onChange={(e) => setField(trLocale, "excerpt", e.target.value)}
                placeholder={watch("excerpt") || ""}
              />
              <CopyFromHrButton
                onClick={() =>
                  setField(trLocale, "excerpt", watch("excerpt") || "")
                }
              />
            </div>

            <div>
              <RichTextEditor
                key={`${trLocale}-${trContentKey}`}
                label="Sadržaj"
                content={getField(trLocale, "content")}
                onChange={(html) =>
                  setField(trLocale, "content", html === "<p></p>" ? "" : html)
                }
              />
              <CopyFromHrButton
                onClick={() => {
                  setField(trLocale, "content", content);
                  setTrContentKey((k) => k + 1);
                }}
              />
            </div>
          </div>
        )}

        {/* Submit */}
        <div className="flex items-center gap-4">
          <Button type="submit" variant="primary" isLoading={isSubmitting}>
            Spremi članak
          </Button>
          <Link href="/dashboard/admin/blog">
            <Button type="button" variant="ghost">
              Odustani
            </Button>
          </Link>
        </div>
      </form>
    </div>
  );
}
