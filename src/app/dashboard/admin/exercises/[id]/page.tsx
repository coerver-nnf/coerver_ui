...
export default function EditExercisePage({
  ...
  }, [selectedCategoryId, subcategories, setValue, initialCategoryId]);

  async function loadData() {
    setLoading(true);
    try {
      const [exercise, categoriesData, subcategoriesData] = await Promise.all([
        getExerciseById(id),
        getExerciseCategories(),
        getExerciseSubcategories(),
      ]);
      ...
      setCoachingPoints(exercise.coaching_points || []);
      setEquipment(exercise.equipment || []);
      setInitialCategoryId(exercise.category_id);

      // Filter subcategories for initial category
      if (exercise.category_id) {
        const filtered = subcategoriesData.filter((s) => s.category_id === exercise.category_id);
        setFilteredSubcategories(filtered);
      }
      ...
      reset({
        ...
        subcategory_id: exercise.subcategory_id || "",
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold text-coerver-gray-900">Uredi vježbu</h1>
      </div>
